...
compounds = ["naftaleen", "acenaftyleen", "acenafteen"]
#compounds = ["fluoreen", "fenantreen", "antraceen"]

# Rename the contaminants to be plotted (optional), put them in the same order as above.
rename = ["naphtalene", "acenaphthylene", "acenaphtene"]
#rename = ["fluorene", "fenantrene", "anthracene"]
...
for i, compound in enumerate(compounds):
    ...
    for j, location in enumerate(plots):
        ...
        if include_ytick:
            ax.set_ylabel(f"{rename[i]}", fontsize = 15)
        else:
            ax.set_yticklabels([])
